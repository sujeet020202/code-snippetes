		...
		switch ($real_type) {
			...
			case 'upload_file':
				...
				else
				{
					...
					$file_url = $this->site_url.$field_info->extras->upload_path."/$value";
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	{
		if(!file_exists($this->root_url.$this->theme_path.$this->theme.'/views/list_template.php'))
		{
			
			throw new Exception($this->root_url.$this->theme_path.$this->theme.'/views/list_template.php The template does not exist. Please check your files and try again.', 12);
			die();
		}
	}
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		$data->add_url				= $this->getAddUrl($this->add_url_path);
		$data->edit_url				= $this->getEditUrl('', $this->edit_url_path);
		$data->delete_url			= $this->getDeleteUrl();
        $data->delete_multiple_url	= $this->getDeleteMultipleUrl();
		$data->read_url				= $this->getReadUrl('', $this->read_url_path);
		...
		$data->status_db_field_name	= $this->data_status_field_name;
		$data->basic_db_table		= $this->basic_db_table;
		$data->bulk_action_url		= $this->bulk_action_url;
		$data->unset_bulk_delete	= $this->unset_bulk_delete;
		$data->unset_bulk_publish	= $this->unset_bulk_publish;
		$data->unset_bulk_unpublish	= $this->unset_bulk_unpublish;
		
		$data->bulk_delete_button_text = $this->bulk_delete_button_text;
		$data->bulk_publish_button_text = $this->bulk_publish_button_text;
		$data->bulk_unpublish_button_text = $this->bulk_unpublish_button_text;
		$data->add_button_text = $this->add_button_text;
	...
	protected function showEditForm($state_info)
	{
		...
		$data->read_url		= $this->getReadUrl($state_info->primary_key, $this->read_url_path);
	...
	{
		$this->css_files[sha1($css_file)] = $this->site_url.$css_file;
	}

	public function set_js($js_file)
	{
		$this->js_files[sha1($js_file)] = $this->site_url.$js_file;
	}

	public function set_js_lib($js_file)
	{
		$this->js_lib_files[sha1($js_file)] = $this->site_url.$js_file;
		$this->js_files[sha1($js_file)] = $this->site_url.$js_file;
	}

	public function set_js_config($js_file)
	{
		$this->js_config_files[sha1($js_file)] = $this->site_url.$js_file;
		$this->js_files[sha1($js_file)] = $this->site_url.$js_file;
	}
	...
	protected function get_upload_file_readonly_input($field_info,$value)
	{
		...
		$value = !empty($value) ? '<a href="'.$file.'" target="_blank2">'.$value.'</a>' : '';
	...
	protected function get_upload_file_input($field_info, $value)
	{
		...
		$input .= $is_image ? " $image_class'><img src='".$file_url."' height='50px'>" : "' target='_blank1'>$file_url";
	...
	protected function setThemeBasics()
	{
		...
		include($this->root_url.$this->theme_path.$this->theme.'/config.php');
	...
	protected function _theme_view($view, $vars = array(), $return = FALSE)
	{
		...
		$view_file = $this->root_url.$this->theme_path.$this->theme.'/views/';
...
class grocery_CRUD_States extends grocery_CRUD_Layout
{
	...
	protected function getAddUrl($custome_url = '')
	{
		if($custome_url == null && $custome_url ==''){
	...
	protected function getEditUrl($primary_key = null, $custome_url = null)
	{
		if($custome_url === null){
			if($primary_key === null)
			...
			return $custome_url.$primary_key;
		}
	}

	protected function getReadUrl($primary_key = null, $custome_url = null)
	{
		if($custome_url === null){			
			if($primary_key === null)
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	protected $read_url_path		= null;
	protected $add_url_path			= null;
	protected $edit_url_path		= null;

	/* The unsetters */
	protected $unset_texteditor		= array();
	
	protected $data_status_field_name = null;
	protected $bulk_action_url		= null;
	protected $unset_bulk_delete	= false;
	protected $unset_bulk_publish	= false;
	protected $unset_bulk_unpublish	= false;
	
	protected $bulk_delete_button_text = null;
	protected $bulk_publish_button_text = null;
	protected $bulk_unpublish_button_text = null;
	...
	{
	$this->site_url =  config_item('site_url');
	$this->root_url =  config_item('root_url');
	
	}


	/**
	 * Set The Custome READ URL Path to read Data
	 * @access	public
	 * @param	string
	 * @return	void
	 */
	public function set_read_url_path($url = '')
	{
		$this->read_url_path = $url;
		return $this;
	}
	
	/**
	 * Set The Custome ADD URL Path to add Data
	 * @access	public
	 * @param	string
	 * @return	void
	 */
	public function set_add_url_path($url = '')
	{
		$this->add_url_path = $url;
		return $this;
	}
	
	/**
	 * Set The Custome EDIT URL Path to edit Data
	 * @access	public
	 * @param	string
	 * @return	void
	 */
	public function set_edit_url_path($url = '')
	{
		$this->edit_url_path = $url;
		return $this;
	}
	/**
	...
	/**
	 * Set the data status db field name to permorf bulk publish/unpublish action in list
	 *
	 * @return	void
	 */
	public function set_data_status_field_name($fields_name='')
	{
		$this->data_status_field_name = $fields_name;

		return $this;
	}
	
	public function set_bulk_delete_button_text($button_text=''){
		$this->bulk_delete_button_text = $button_text;
		return $this;
	}
	public function set_bulk_publish_button_text($button_text=''){
		$this->bulk_publish_button_text = $button_text;
		return $this;
	}
	public function set_bulk_unpublish_button_text($button_text=''){
		$this->bulk_unpublish_button_text = $button_text;
		return $this;
	}
	public function set_add_button_text($button_text=''){
		$this->add_button_text = $button_text;
		return $this;
	}
	
	/**
	 * Set the bulk action URL in the list
	 *
	 * @return	void
	 */
	public function set_bulk_action_url($url='')
	{
		$this->bulk_action_url = $url;
	 ...
	 * @return	void
	 */
	public function unset_bulk_delete()
	{
		$this->unset_bulk_delete = true;

		return $this;
	}
	
	/**
	 * Unsets the publish operation from the list
	 *
	 * @return	void
	 */
	public function unset_bulk_publish()
	{
		$this->unset_bulk_publish = true;

		return $this;
	}
	
	/**
	 * Unsets the unpublish operation from the list
	 *
	 * @return	void
	 */
	public function unset_bulk_unpublish()
	{
		$this->unset_bulk_unpublish = true;

		return $this;
	}

	/**
	 * Unsets all the bulk operations from the list
	 *
	 * @return	void
	 */
	public function unset_bulk_operations()
	{
		$this->unset_bulk_delete = true;
		$this->unset_bulk_publish = true;
		$this->unset_bulk_unpublish = true;
	...
	/**
	 * Unsets all the operations except add, edit and read from the list
	 *
	 * @return	void
	 */
	public function unset_operations_except_add_edit_read()
	{
		$this->unset_bulk_delete = true;
		$this->unset_bulk_publish = true;
		$this->unset_bulk_unpublish = true;
	...
	protected function _load_language()
	{
		...
		include($this->root_url.$this->default_language_path.'/'.$this->language.'.php');
	...
	public function set_field_upload($field_name, $upload_dir = '', $allowed_file_types = '')
	{
		...
		//echo '--'.FCPATH;
		if (!is_dir($this->root_url.$upload_dir)) {
...
class UploadHandler
{
    ...
    function __construct($options=null) {
        ...
        $this->options = array(
			...
			'upload_url' => config_item('root_url').'/files/',
            'param_name' => 'files',
    ...
    private function _transliterate_characters($file_name)
	{
		...
		include($this->root_url.$this->default_config_path.'/translit_chars.php');
        ...
        if (!$error && $file->name) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    move_uploaded_file($uploaded_file, $this->root_url.$file_path);
                }
            } else {
            ...
            $file_size = filesize($this->root_url.$file_path);
            if ($file_size === $file->size) {
            		if ($this->options['orient_image']) {
            		    $this->orient_image($file_path);
            		}
                $file->url = $this->options['upload_url'].rawurlencode($file->name);
				$file->url = $this->site_url.$file_path;
                foreach($this->options['image_versions'] as $version => $options) {
